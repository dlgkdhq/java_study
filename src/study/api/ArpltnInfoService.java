...
public class ArpltnInfoService {
	...
	public MinuDustWeekDTO getMinuDustWeek() {
		...
		try {
			...
			mdwDTO.setTotalCount(objectToInt(body.get("totalCount"))); //메소드 만들어서 처리해도 됨!
			//mdwDTO.setFrcstOneCn(item.get("frcstOneDt").toString());
			mdwDTO.setFrcstOneCn(valueToString(item.get("frcstOneCn").toString()));
			mdwDTO.setFrcstTwoCn(valueToString(item.get("frcstTwoCn").toString()));
			mdwDTO.setFrcstOneDt(valueToString(item.get("frcstOneDt").toString()));
			mdwDTO.setFrcstTwoDt(valueToString(item.get("frcstTwoDt").toString()));
			mdwDTO.setPresnatnDt(valueToString(item.get("presnatnDt").toString()));
