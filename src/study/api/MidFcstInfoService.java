...
public class MidFcstInfoService {
	...
	public MidTaDTO findMidTa(String tmFc, String regId) {
		...
		MidTaDTO midTaDTO = null;	//null이 리턴되면 잘못됐다고 생각할거임
		...
		try {
			...
			JSONObject jsonObj = (JSONObject)jsonParser.parse(jsonString);	//jsonParser.parse(jsonString) : jsonString을 변환해줌
								//객체 타입으로 형변환
			
			JSONObject response = (JSONObject)jsonObj.get("response"); //"response" 로 부르고 결과를 객체로 형변환해서 받는다
			
			JSONObject header = (JSONObject)response.get("header");
			
			//header.get("resultCode");
			//header.get("resultMsg");
			System.out.println(header.get("resultCode"));	//헤더 안에 있는 key:value 에 바로 접근 가능
			...
			JSONArray item = (JSONArray)items.get("item");	//items 객체 안에 item 배열이 있음
			
			/*
			// 배열 안에 데이터가 여러개일 경우 반복문으로 처리 가능
